#slightly modified crf.py of nlkt.tag by Kathrin
...
class CRFTagger(TaggerI):
    ...
    def _get_features(self, tokens, idx):
        ...
        if len(token) > 0:
            feature_list.append('SUF1_' + token[-1:]) 
        if len(token) > 1: 
            feature_list.append('SUF2_' + token[-2:])    
        if len(token) > 2: 
            feature_list.append('SUF3_' + token[-3:])
            
################# added by Kathrin ###########################################            

        #additional suffixes
        if len(token) > 3:
            feature_list.append('SUF4_' + token[-4:])
        if len(token) > 4:
            feature_list.append('SUF5_' + token[-5:])

        
        # Prefix up to length 4
        if len(token) > 0:
            feature_list.append('PRE1_' + token[:1])
        if len(token) > 1:
            feature_list.append('PRE2_' + token[:2])
        if len(token) > 2:
            feature_list.append('PRE3_' + token[:3])
        if len(token) > 3:
            feature_list.append('PRE4_' + token[:4])
        if len(token) > 4:
            feature_list.append('PRE5_' + token[:5])

        #previos and next Word
        if idx>0:
            feature_list.append('PRV_' + tokens[idx-1][0])
        if len(tokens)>(idx+1):
            feature_list.append('NXT_' + tokens[idx+1][0])
...
#############################################################################
    ...
    def train(self, train_data, model_file):
        ...
        for sent in train_data:
            ...
            features = [self._feature_func(sent,i) for i in range(len(sent))]
