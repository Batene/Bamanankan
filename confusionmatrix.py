...
##### added by Kathrin #####################################################
    def get_values_not_null(self):
        values = []
        for i in range(len(self._confusion)):
            for j in range(len(self._confusion[i])):
                if self._confusion[i][j] != 0:
                    values.append((self._values[i], self._values[j],self._confusion[i][j]))
        #print(values)
        return values
############################################################################                    
...
def demo():
    ...
    print(ConfusionMatrix(reference, test)._confusion)
    print(ConfusionMatrix(reference, test).get_values_not_null())
